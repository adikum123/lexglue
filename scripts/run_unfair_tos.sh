...
CUDA_VISIBLE_DEVICES=${GPU_NUMBER} python ${URL}/experiments/unfair_tos.py \
--model_name_or_path ${MODEL_NAME} --do_lower_case ${LOWER_CASE} --max_train_samples 30 --max_predict_samples=10 \
--pad_to_max_length --output_dir logs/${TASK}/${MODEL_NAME}/seed_1 --do_train --do_pred \
--load_best_model_at_end --metric_for_best_model micro-f1 \
--greater_is_better True --evaluation_strategy epoch --save_strategy epoch --save_total_limit 5 \
--num_train_epochs 3 --learning_rate 3e-4 --per_device_train_batch_size ${BATCH_SIZE} \
